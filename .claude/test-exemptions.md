...
| Win10 捆绑 conhost（OpenConsole 1.24）启动握手 DA1 的端到端行为 | 本机 Win11 inbox conhost 不发该 DA1——差异只在 Win10 捆绑路径触发，自动化不可达 | L1 纯函数全分支（`strip_da1_queries`/`split_trailing_da1_prefix`/`apply_output_strip`/`inject_da1_response` 每次必答回归）+ L2 就绪闸门用例 + Win10 实机人工验收三点（新建终端无蜂鸣无 `[?1;2c` / 双击恢复无前缀污染 / claude 功能正常） | 2026-09-18 ADR-0022 DA1 接管登记 |
