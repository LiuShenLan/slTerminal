...
| background-tasks.e2e.ts 用例 F 真实 tick 时序豁免（E2E-03） | 用例全程依赖真实链路（set_config 原子写盘 + scheduler 真实 2s tick + 后端扫描 + 导航树计数回显），断言窗口（落盘 10s / 计数 20s / 勾选态 8s）与真实调度竞争；2026-08-31 与 2026-09-01 验收各失败 1 次、共通过 1 次，失败环节各异（启用落盘超时 / 启用后计数不变 / 重试勾选态不回显）——测试命名工程 5 commit（7229326/08d8c07/2eff23f/616a285/20503f2）对用例与相关实现零行为接触（git diff 实证），判定既有 flaky 非本次引入 | 用例 E 同链路真实 tick 断言（同链路通过即扫描健康）+ 调度器 L2 用例（`background-tasks-scheduler.test.ts`）+ WDIO_RETRIES=1 用例级重试兜底 + 失败后复跑验收 | E2E-03 |
