...
- `docs/<task>/workflows/stage-NN-*.js`（Stage 脚本）+ `fix-loop.js`（修复循环）+ `verify/stage-NN.md`（断言）
...
- **修复点现状代码**：Read 实读原文（行号 + 代码形态），禁凭 review 报告转述——报告与现状有漂移即按现状修订清单，并在条目留痕（实证：报告称嵌套场景未覆盖，实读测试文件发现已固化→该项降为验证项；报告称 3 处 catch{}，实读为 5 处→按 5 处列项）。凭报告转述写清单 = 把失实传导给执行 agent
...
**产出**：`docs/<task>/checklist.md`（如 `refactoring-checklist.md`、`debug-checklist.md`，按任务性质命名）——按优先级（P0-P4）+ 模块组织的完整清单，每项含 ID + 六段式明细（见下「清单条目详细度标准」）。50-80 项均属正常规模。

#### 清单条目详细度标准（弱执行者可照抄——强制六段式）

计划的执行 agent 能力弱于计划者：每项修复必须是「照做即可」粒度，**禁止「修复要点」级摘要**（实证：要点级 plan 被用户以「计划的执行者的能力会弱一点」整版拒绝重写；六段式版本一次性通过）。每项强制六段：

1. **位置**：精确 `file:line`（计划期实读核验——全部修复点的现状代码必须在写清单前 Read 原文，见上方「事实核验」）
2. **现状**：现状代码摘录（关键行原文，让执行 agent 免回查即可理解改什么）
3. **修复步骤**：编号步骤，含**可直接照抄的修改后代码块**——执行 agent 不设计代码，只抄写适配；设计判断（分支语义/守卫条件/常量取值）全部在计划期完成并写死进步骤
4. **测试同步**：改/加哪个测试文件、什么用例（含用例名建议）；既有用例需适配的逐一点名
5. **文档同步**：哪个 CLAUDE.md/文档哪节、改成什么口径
6. **验证**：机械可执行断言（grep 模式 / 命令退出码 / 测试通过）

六段同样约束**呈现给用户的 plan 文件**：plan 应包含 checklist/stages 的产物级完整草案（每项六段、每 Stage 分工表+断言），批准即落盘——禁止「plan 只列骨架、批准后再扩写」（二次扩写等于计划做了两遍，且骨架版无法让用户判断详细度是否足够）。
...
2. 清单每项六段式齐全（位置 / 现状摘录 / 编号步骤含可照抄代码块 / 测试同步 / 文档同步 / 验证断言）——抽查 ≥3 项逐段核对（要点级摘要 = 返工，见「清单条目详细度标准」）
3. 依赖库行为的条目有一手证据（源码 `file:line` 或官方文档）
4. 共享集合改动的消费方清单含测试文件（grep 断言字面量）
5. 各 Stage 分工表文件清单 = 对应脚本 prompt 触碰文件全集
6. verify 每条断言与"该 Stage 完成后的真实中间态"一致（计数/枚举类按中间态推导，不照抄终态）
7. verify 每条断言的核实数据在本 Stage 门禁命令产出内，或写明静态取数口径
...
主 agent 使用时：读取模板 → 替换 `__PLACEHOLDER__` → 落盘 `docs/<task>/workflows/stage-NN-*.js` → 语法预检（见 5.1）。
...
| `__VERIFY_FILE__` | 断言文件路径（如 `docs/<task>/workflows/verify/stage-01.md`） |
...
**Red flags（踩坑实录，出现即停）：**
- 清单只写「修复要点」级摘要——执行 agent 能力弱，每项必须六段式（位置 file:line / 现状代码摘录 / 编号步骤含可照抄代码块 / 测试同步 / 文档同步 / 验证断言）；要点级 plan 曾被用户整版拒绝重写
- 凭 review 报告转述写清单（行号/代码形态不实读）——报告漂移会传导给执行 agent（实证：报告失实致计划项误列，实读翻案；漏报 2 处 catch{}，实读补正）
...
- 脚本/verify 路径单复数或相对/全路径混用——统一 `docs/<task>/workflows/` 全路径一次写对；改名牵连全量引用替换
