...
4. **GitHub issue 引用须核查正文 + 关键评论**：状态（open/closed）、collaborator 的修复声明（可能藏在评论中，早于检索日）、其他用户补充的数值/建议，优先用 GitHub API（`api.github.com/repos/{owner}/{repo}/issues/{n}` + `/comments`）
5. 不得 spawn 任何子代理——你是叶子节点，全部检索亲自完成
...
- [ ] **精确示例/引文/字段清单不以「搜索摘要」为唯一来源**？须 WebFetch 原文逐字确认；无法确认的示例删除，或标注"形态由官方规则推导"而非伪称来源
- [ ] **标注「未经核验」的信息（日期/状态/数字）要么执行核验、要么从文档删除**？标注不能替代核验——带"未经核验"字样的信息本身就是可疑项
- [ ] **来源清单条目标注获取方式**（全文抓取/搜索摘要）？**过程统计（关键词数/fetch 数）与来源清单可对应**？无法对应的数字不写
- [ ] 同一概念全文各处表述一致，无内部矛盾？
