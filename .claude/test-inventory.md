...
| `src/__tests__/terminal.test.tsx` | 14 | TerminalPanel：loading 遮罩 1.5s 超时（TRM-05）/Windows build/spawn/active=false 标题恢复/customTitle 挂载恢复 + onDidParametersChange 同步（F8）/tabLogo 状态机（logoRef 命中保持/清空/双清——CLI logo 引用改指 profile 注册表，F9） |
...
| `src/__tests__/cli-profile-claude.test.ts` | 53 | claude profile 身份域（MC-104）：side-effect 注册/CLAUDE_CLI_ID 常量与注册一致性/身份域字段完整（含 hooks 七字段 + history 三字段）/capabilities.history 三字段齐备（supportsFork/buildResumeCommand/buildRestoreInput，MC-315/316）/带参命中/`_reset` 恢复（9——含 **configEditor 挂载 = ClaudeHooksConfigEditor（KZ-1，代 editor-dispatch 登记 +1）** 与 **configLayers 三层现值（KZ-4：user/project/local + label/hint，层切换器数据源，Stage 04 review-fix +1）**）——语义并入自退役规则用例（6）+ hooks 策略（Stage 02，35）：eventToStatus 26 用例语义迁入（原状态映射测试语义，10 事件 × notificationType + STATUS_EMOJI 联合守卫 + AgentStatus 类型兼容）+ classifyNotification 五映射表驱动 9 条（NAH-03 迁入，MC-422）+ **history 策略（Stage 05，7）+ AQ-1 单引号回归（Stage 01，1）：buildResumeCommand 4（有 cwd/无 cwd/与迁出源逐字一致/cwd 含单引号按 PowerShell 规则转义 `''`——AQ-1）+ buildRestoreInput 4（普通/fork 追加/`\r` 结尾无 `\r\n`/无 cwd 可注入——输出与迁出源逐字一致，断言漂移即实现有误）** |
| `src/__tests__/mock-cli-profile.test.tsx` | 12 | **AC-4 mock profile 全链路验收（Stage 07，夹具 helpers/mockCliProfile；Stage 05 review-fix KZ-7 代 l2-mock-editor 登记：AC-4④ 重写为双向分派断言，用例数保持 12——单条 it 内双向断言，若 l2-mock-editor 拆分 it 致数变化以终验 vitest 实跑为准）**：① OSC 133 命中——matchByCommand("mockcli --flag") 命中 → 页签标题/logo（profile.tabTitle/iconSrc）+ agentSession.cliId 写入（useCommandDetection 链路，真实注册表 + 真实 TerminalRegistry；未命中零副作用 + OSC 133 D 清会话）② hooks 能力被真实调用——eventToStatus 经 useXterm 事件路径（spy 入参 event+notificationType + 四态/会话写入）+ classifyNotification 经通知调度路径（spy 入参 payload + toast 派发）③ 历史聚合 UI——mock 条目（AgentHistorySession cliId="mockcli"）出现在历史区 + 行 logo 按 session.cliId 取 mockcli iconSrc ④ hub 选择行——两枚按钮（claude + mockcli，均 hasConfigEditor=true）+ **KZ-7 双向分派断言：选中 mockcli → 渲染 mock 编辑器桩（data-e2e="mockcli-config-editor" 标记）+ 未渲染 ClaudeHooksConfigEditor（JsonMode mock 零调用）；选中 claude → JsonMode 被调用 + 桩标记不存在** + selectedCli 持久化（updateParameters + 显式 onLayoutChange/toJSON）与挂载恢复 ⑤ 恢复注入——pty.write 内容 = mock buildRestoreInput 桩输出（"mockcli --resume" 可识别前缀，普通/fork）+ addPanel title = profile.tabTitle |
