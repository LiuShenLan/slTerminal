...
**iframe 内字体加载态不可自宿主页读取（TE-08 实证定案，2026-09-09）**：宿主页 `document.fonts` 不覆盖 iframe srcdoc 文档（实测 hostSize=0 且 `fonts.check` 对任意族名恒 true——对照组 `NoSuchFontXyzQq` 亦 true，check 语义失效；iframe opaque origin 宿主不可读），KaTeX 字体真实加载锚点必须自 iframe 内取——注入的 `fontProbe` 段在 iframe 内 `fonts.ready` 后 `check('12px "KaTeX_Main"')` 上行宿主桥 → 主窗 `window.__slterm_e2e_fontProbe`（E2E 门控，生产零注入面），markdown.e2e.ts 以「族名已声明 + probe true」双前置断言。
