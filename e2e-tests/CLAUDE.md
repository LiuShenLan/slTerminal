...
### 预览宿主 iframe 驱动契约（ADR-0021，2026-09-13 定稿）

预览渲染于主窗内跨源沙箱宿主 iframe（`iframe[data-e2e="preview-frame-<panelId>"]`，src = 自定义协议宿主页）——app 恒单窗口，旧「多 webview switchToWindow」契约（句柄 = label = preview-<panelId>）整体退役。spike（2026-09-12）对 frame 内驱动能力的实测结论：

| 问 | 结论 | 证据 |
|---|---|---|
| ① 子帧自定义协议加载 | **yes** | 主窗 CSP `frame-src http://slterm-preview.localhost` 收窄放行；宿主页加载、桥脚本执行（host_ready 上行实证） |
| ② 嵌套 srcdoc + meta CSP 继承 | **yes** | 内联脚本执行、data: img 加载（域级 CSP meta 继承 W3C 行为） |
| ③ 双向 postMessage | **yes** | origin 实证 = "null"（宿主 opaque origin——主窗→宿主 targetOrigin 只能 `"*"`，宿主→主窗校验 = source 归属 + origin "null"） |
| ④ **frame 内 execute（switchToFrame）** | **no（全灭）** | embedded driver 不接线**任何**非顶层上下文——同源 about:blank 对照帧同样 execute 超时，与跨源/sandbox 无关（driver 能力缺陷，非本 app 约束） |
| ⑤ data: 字体嵌套加载 | **yes** | KaTeX data: 字体真实加载（fontProbe=true） |

**④ 的处置 = 探针模式（Q4 用户裁决）**：frame 内 execute 不可达 → 内容/加载断言走「主窗 E2E 全局」探针（fontProbe 同款形态）——PreviewFrame 经 E2E_ENABLED 门控写 `window.__slterm_e2e_previewDoc`（panelId → 最近推送产物）与 `__slterm_e2e_iframeLoaded`（panelId → 加载完成计数）；specUtils `waitPreviewDocContains` = 两探针组合（推送产物含 X + 桥受理加载完成——等价旧「宿主页 srcdoc 属性」断言强度）。宿主 iframe 元素本身在主窗 DOM 可读（挂载/移除断言 = 主窗 querySelector）；keyfwd 链路经 fixture script 合成 keydown 做真实往返（html.e2e 正/负两用例）。

**iframe 内字体加载态不可自宿主页读取（TE-08 实证定案，2026-09-09）**：宿主页 `document.fonts` 不覆盖 iframe srcdoc 文档（实测 hostSize=0 且 `fonts.check` 对任意族名恒 true——对照组 `NoSuchFontXyzQq` 亦 true，check 语义失效；iframe opaque origin 宿主不可读），KaTeX 字体真实加载锚点必须自 iframe 内取——注入的 `fontProbe` 段在 iframe 内 `fonts.ready` 后 `check('12px "KaTeX_Main"')` 上行（经宿主桥 relay）→ 主窗 `window.__slterm_e2e_fontProbe`（E2E 门控，键 = panelId，生产零注入面），markdown.e2e.ts 以「族名已声明 + probe true」双前置断言。
...
Node runner 侧公共件（fs/os 操作）收 `node-helpers.ts`——与浏览器侧 `helpers.ts`（VITE_E2E 打包进前端，禁 Node API）分工，Node API 禁止进 `helpers.ts`（TE-06）。
...
**防复发校验（CP-046 键级断言）**：覆盖 USERPROFILE 前对真实屋快照——`~/.claude/settings.json` 取哨兵键（`hooks`/`statusLine`/`env`，本套件全部写入面——TE-04）做存在性 + 值快照，exit 时键级比对，不做整文件 diff；`~/.slterminal/statusline-backup.json` 维持文件 sha256、`~/.slterminal/hooks/` 维持整树快照、`~/.slterminal/hooks-events/` 仅当启动时不存在才校验 exit 仍不存在（存在 = 用户会话在用，跳过防误报）。任何泄漏（Rust 收敛遗漏/新裸 `dirs::home_dir()` 消费点）独立报红并 `exitCode = 1`。哨兵键集合 = run-wdio.cjs `SETTINGS_SENTINEL_KEYS`——新增 settings.json 消费点时须同步向该数组加键（登记进本防复发节口径）。
...
**已知并发误报面（已收窄，CP-046 + TE-04）**：开发者跑 e2e 的同时自己开真实 claude/slterminal 改写非哨兵键（permissions/用户配置）仍合法放行；改写 `env` 键（如手动改 `ANTHROPIC_*` 配置）会命中哨兵报红——已知限制（TE-04），e2e 运行期间勿动。并发 hooks 注入/卸载（写 `hooks`/`statusLine` 键）仍会命中哨兵键报红——属真实泄漏信号，排查方向照旧（Rust 侧 home 解析收敛），不静默。
...
- **`$()`/elementClick 触发 tauri-service 焦点检查（focusCommands）**：`$`/`$$`/`findElement`/`findElements`/`elementClick`/`getTitle` 命令前 `ensureActiveWindowFocus` 经 core.invoke 查窗口状态，查询不可用（WARN "core.invoke not available after 5s timeout"）时每命令 +5-15s（2026-09-06 实测：cli-aliases 真实手势版步骤 2 四个 focus 命令吃 40-60s，长链用例被拖出 mocha 60s 上限多轮失败）；executeScript **豁免**不触发。spec 编写优先 execute 内 helper；新引入 `$()` 元素命令前评估焦点检查成本。运行前提 = 窗口前台聚焦——wdio.conf beforeSuite TQ-E-10 探针 fast-fail 保证（首 worker 失焦即报错退出，多 spec 形态非首 worker 降级 warn 继续——TE-05）；**即便探针通过，每 focus 命令仍确定性 +5s**（`__TAURI_INTERNALS__.core` 缺失致窗口态查询恒失败，与聚焦无关——2026-09-08 修正段口径为准），cli-aliases 已回归真实手势（CP-030，预算经 suite 级 this.timeout(120000) 承接）。**2026-09-08 排查修正**：聚焦前提满足（TQ-E-10 通过）时窗口态查询通道仍可能不可用——本 app 构建页面 `__TAURI_INTERNALS__` 无 `.core` 成员，tauri-service 以 `__TAURI_INTERNALS__.core.invoke("plugin:wdio|get_window_states")` 查询恒失败（实测 TypeError + 5s 超时 WARN），每 focus 命令确定性 +5s、与窗口是否聚焦无关；cli-aliases 真实手势长链实测单轮 13 次 WARN（含 suite 清理）≈ +65s，经 suite 级 `this.timeout(120000)` 放宽预算（见失败排查提示节 timeout 生效形态）。新增含 `$`/click 的长链 spec 须照此预算。**WARN 可观测化（TE-09）**：run-wdio 结束打印 `[wdio-launcher] tauri-service core.invoke WARN 计数 = N` 行；基线 = **全量 e2e（15 spec）0 次、cli-aliases 单 spec 0 次（2026-09-09 实测）**——计数显著超 0 = focus 命令面扩大或抑制失效，排查 `$`/click 新增点。**口径复核留痕（2026-09-09）**：本节 2026-09-08 记录的「cli-aliases ≈ 13 次 WARN、每 focus 命令确定性 +5s」在当前代码态未复现（本轮 focus 命令间隔 ~10ms、无超时惩罚；疑因 beforeSuite 显式 `switchToWindow('main')` 命中 tauri-service userSwitchedWindowCache 抑制）——历史结论保留，生效性待后续复核。
...
- **定向运行官方形态 = 单 `--spec`**（2026-09-08 实证）：`run-wdio.cjs --spec a.e2e.ts --spec b.e2e.ts` 会为每个 spec 起独立 worker/独立应用实例——后起的应用实例在 Windows 前台锁定期内无法取得前台焦点，beforeSuite TQ-E-10 探针首 worker fast-fail、第 2 位起的 worker 降级 warn 继续（TE-05——不再确定性失败），但其 `$` 族命令全吃 +5s/命令 focus 惩罚（长链 spec 会拖出 timeout 预算）——仍推荐单 spec 分次/全量 config 数组形态；config `specs` 数组全量形态 = 单 worker 单 app 会话内顺序跑各 spec（无重启聚焦问题，S12 全量回归走此形态）。多 spec 验证请分次单 spec 调用。
- **选择器**：`data-e2e` 属性。
...
| 预览渲染往返（宿主桥 relay/iframe 执行/keyfwd 消费） | jsdom 不加载跨源 src、不执行宿主页脚本——L2 止于 postMessage 构造边界 + Rust 字符串断言 | `html.e2e.ts`/`markdown.e2e.ts` 真实二进制 + 探针模式（ADR-0021 驱动契约）+ L2 负面用例 |
