...
//
// WRK-11②：补"SidebarTree 实际使用 makeEmptyLayout"断言——渲染真实 SidebarTree，
// 行为断言新建项目/页面的 layout 为 makeEmptyLayout 产物（空对象）。
// 注：makeEmptyLayout 是 SidebarTree.tsx 同文件局部函数（L352/L381 直接调用），
// vi.mock 替换模块导出拦截不到局部函数引用——故以行为断言替代调用点 spy。

import { describe, it, expect, beforeEach, vi } from "vitest";
import React from "react";
import { render, fireEvent, waitFor, cleanup } from "@testing-library/react";

// ─── Hoisted：dialog mock + pageApis mock ───
const mocks = vi.hoisted(() => {
  const mockOpenDialog = vi.fn();
  const mockOpenHooksConfigPanel = vi.fn();
  return { mockOpenDialog, mockOpenHooksConfigPanel };
});
...
import { makeEmptyLayout, default as SidebarTree } from "../features/sidebar/SidebarTree";
...
// ─── WRK-11②：SidebarTree 实际使用 makeEmptyLayout（真实组件 + 真实 stores）───
...
describe("SidebarTree 实际使用 makeEmptyLayout（WRK-11②）", () => {
  beforeEach(() => {
    cleanup();
    mocks.mockOpenDialog.mockReset();
    mocks.mockOpenHooksConfigPanel.mockReset();
  ...
  it("T9: 新建项目流程产出 makeEmptyLayout 产物——page.layout 为空对象", async () => {
    ...
    // SidebarTree.handleAddProject 经 makeEmptyLayout()（L352）构造 layout，
  ...
  it("T10: 右键新建操作页面同样产出空布局（makeEmptyLayout 契约）", () => {
    ...
    // handleNewPage L381 经 makeEmptyLayout() 构造）
