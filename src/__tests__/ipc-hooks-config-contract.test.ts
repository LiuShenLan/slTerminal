// ipc-hooks-config-contract.test.ts — hooks 配置 IPC wrapper 合约测试（P3-FE-05，IHE-06 工厂化）
//
// 照 ipc-hooks-contract.test.ts 模式，经共享工厂 describeIpcContract（helpers/ipc-contract.ts）
// 声明式驱动两命令 × 四维（命令名 / 参数结构 / 正常返回 / 异常传播）= 12 条用例：
...
describeIpcContract("readHooksConfig 合约（hooks_config_read）", [
  // 维度 1：命令名
  {
    name: "应调用 hooks_config_read 命令",
    cmd: "hooks_config_read",
    call: () => hooksConfig.readHooksConfig("user"),
    respond: null,
  },
  ...
  // 维度 2：参数结构——project 层带 projectPath
  {
    ...
    cmd: "hooks_config_read",
    call: () => hooksConfig.readHooksConfig("project", "D:/repo"),
    respond: null,
    expectArgs: { layer: "project", projectPath: "D:/repo" },
    expectExactKeys: ["layer", "projectPath"],
  },
...
describeIpcContract("writeHooksConfig 合约（hooks_config_write）", [
  // 维度 1：命令名
  {
    name: "应调用 hooks_config_write 命令",
    cmd: "hooks_config_write",
    call: () => hooksConfig.writeHooksConfig("user", {}),
    respond: undefined,
  },
  // 维度 2：payload 键集合精确匹配——带 projectPath（local 层）
  {
    ...
    cmd: "hooks_config_write",
    call: () => hooksConfig.writeHooksConfig("local", mockSubtree, "D:/repo"),
    respond: undefined,
    expectArgs: { layer: "local", hooks: mockSubtree, projectPath: "D:/repo" },
    expectExactKeys: ["hooks", "layer", "projectPath"],
  },
  // 维度 2：payload 键集合精确匹配——无 projectPath（user 层）
  {
    name: "user 层调用 payload 键集合精确为 { layer, hooks }（无 projectPath）",
    cmd: "hooks_config_write",
    call: () => hooksConfig.writeHooksConfig("user", { SessionStart: [] }),
    respond: undefined,
    expectArgs: { layer: "user", hooks: { SessionStart: [] } },
    expectExactKeys: ["hooks", "layer"],
  },
