...
// （ADR-0021 预览回迁主窗 DOM 后重写——渲染内容经宿主 iframe host_content
//  推送，断言 = 宿主 contentWindow postMessage 捕获）
//
...
//   7. keepZoom/keepScrollRatio：iframe_loaded 上行后下行恢复
//
// 注：预览内容渲染于主窗 DOM 内跨源沙箱 iframe（jsdom 真实挂载 iframe 元素
// 但不加载跨源 src、不执行宿主页桥脚本）——桥行为由 Rust 侧字符串断言 +
// L4 E2E 验收；本文件在 window message 事件边界上测主窗侧编排与消息桥。
...
/** md 面板默认 panelId（宿主 iframe data-e2e = preview-frame-<panelId>） */
const PANEL_ID = "md-1";
...
/** 宿主 contentWindow postMessage 捕获（下行/推送断言）——按 contentWindow
 *  缓存（重复 spyOn 同属性会叠加包装，iframe 重建自动换新） */
...
function dispatchUplink(frame: HTMLIFrameElement, msg: Record<string, unknown>) {
...
async function sendHostReady(frame: HTMLIFrameElement) {
  await act(async () => dispatchUplink(frame, { type: "slterm_host_ready" }));
}

/** 各次 host_content 推送的装配产物序列（渲染管线产物落地判定——jsdom 不
 *  执行宿主页脚本，断言 = postMessage 载荷捕获） */
function pushedDocs(panelId = PANEL_ID): string[] {
  return hostPostSpy(frameOf(panelId))
    .mock.calls.map(([m]) => m as { type?: string; html?: string })
    .filter((m) => m.type === "slterm_host_content")
    .map((m) => m.html!);
}

/**
 * 等待某次 host_content 推送产物包含指定内容。返回该产物串。
 */
...
describe("MarkdownPanel", () => {
  ...
  it("默认形态 edit：切换条三态 + 无预览宿主 iframe（CM 桥挂载）", async () => {
    ...
    // edit 形态：CM 容器在，无预览宿主（PreviewFrame 未挂载）
  ...
  it("切 preview → CM pane 隐藏保活（container 恒非 null）；渲染产物推送宿主 iframe", async () => {
    const { container } = renderPanel();
    await waitForCmMounted();
    ...
    // 宿主 iframe = 本面板预览宿主（data-e2e = preview-frame-<panelId>）
    expect(frameOf().dataset.e2e).toBe(`preview-frame-${PANEL_ID}`);
  ...
  it("VITE_E2E=1：预览装配追加 fontProbe 段（TE-08 字体加载锚点）", async () => {
    ...
    try {
      ...
      await waitForPushedDoc("slterm_font_probe");
    } finally {
      vi.unstubAllEnvs();
    }
  });
  ...
  it("草稿防抖渲染：edit 击键 300ms 后 preview 内容更新（fake timers）", async () => {
    ...
    try {
      ...
      const docsBefore = pushedDocs();
      expect(docsBefore[docsBefore.length - 1]!).toContain("磁盘内容");
  ...
  it("keepZoom/keepScrollRatio：iframe_loaded 后按镜像下行恢复（zoom_set/scroll_set）", async () => {
    ...
    // 缩放/滚动上行 → 父侧镜像
    await act(async () => {
      dispatchUplink(frame, { type: "slterm_zoom", nonce, zoom: 1.3 });
    });
    await act(async () => {
      dispatchUplink(frame, { type: "slterm_scroll", nonce, ratio: 0.5 });
    });
    // 上行不触发下行恢复（仅 iframe_loaded 触发）
    const downsBefore = post.mock.calls.filter(
      ([m]) => (m as { type?: string }).type !== "slterm_host_content",
    );
    expect(downsBefore.length).toBe(0);

    // 内容重建完成 → 归 1 静默复位 + 按镜像下行恢复（keepZoom/keepScrollRatio 开）
    await act(async () => {
      dispatchUplink(frame, { type: "slterm_iframe_loaded" });
    });
    const downs = post.mock.calls
      .map(([m]) => m as { type?: string; nonce?: string; zoom?: number; ratio?: number })
      .filter((m) => m.type === "slterm_zoom_set" || m.type === "slterm_scroll_set");
  ...
  it("split 形态：双 pane 布局（CM + 宿主 iframe）", async () => {
    const { container } = renderPanel();
    await waitForCmMounted();
    await act(async () => {
      fireEvent.click(container.querySelector('[data-e2e="markdown-mode-split"]')!);
    });
    // split：预览内容推送（PreviewFrame 挂载于右 pane——宿主 iframe 出现 +
    // 桥就绪后 host_content 推送）
    await waitForPushedDoc("磁盘内容");
    const cmCalls = mocks.mockUseCodeMirror.mock.calls;
    const last = cmCalls[cmCalls.length - 1]![0] as { container: unknown };
    expect(last.container).not.toBeNull();
