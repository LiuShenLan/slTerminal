...
// ── 辅助：构造两个视图均在上区的 zones（nav = 生产默认第一槽，NAV-05）──
function zonesBothTop(): Zones {
  return { top: ["nav", "explorer"], bottom: [] };
}
...
describe("toggleViewPure", () => {
  it("已关闭的视图点击后打开（R1）", () => {
    ...
    const result = toggleViewPure(zones, open, "nav");
    expect(result).toEqual({ top: "nav", bottom: null });
  });

  it("已打开的视图点击后关闭（R2）", () => {
    const zones = zonesBothTop();
    const open: OpenState = { top: "nav", bottom: null };
    const result = toggleViewPure(zones, open, "nav");
  ...
  it("打开新视图时隐式关闭同区旧视图（R1 覆盖）", () => {
    ...
    // explorer 代替 nav，nav 不出现在返回中（无历史记忆）
  ...
  it("下区视图的开关独立于上区", () => {
    const zones: Zones = { top: ["test-view"], bottom: ["explorer"] };
    const open: OpenState = { top: "test-view", bottom: null };
    const result = toggleViewPure(zones, open, "explorer");
    expect(result).toEqual({ top: "test-view", bottom: "explorer" });
  });
});
...
describe("moveButtonPure", () => {
  it("跨区 + 视图打开中：跟随并替换目标区旧视图（R6）", () => {
    // P 在上区打开，E 在下区打开
    const zones: Zones = { top: ["test-view"], bottom: ["explorer"] };
    const open: OpenState = { top: "test-view", bottom: "explorer" };
    // 拖拽 test-view 到下区 index 0
    const result = moveButtonPure(zones, open, "test-view", "bottom", 0);
    expect(result.zones).toEqual({ top: [], bottom: ["test-view", "explorer"] });
    // test-view 跟随到下区，explorer 被关闭
    expect(result.open).toEqual({ top: null, bottom: "test-view" });
  });

  it("跨区 + 视图打开中 + 目标区空：跟随到空区", () => {
    const zones: Zones = { top: ["test-view"], bottom: [] };
    const open: OpenState = { top: "test-view", bottom: null };
    const result = moveButtonPure(zones, open, "test-view", "bottom", 0);
    expect(result.zones).toEqual({ top: [], bottom: ["test-view"] });
    expect(result.open).toEqual({ top: null, bottom: "test-view" });
  });

  it("跨区 + 视图未打开：仅归属变化，展示不变（R7）", () => {
    const zones: Zones = { top: ["test-view", "explorer"], bottom: [] };
    const open: OpenState = { top: null, bottom: null };
    const result = moveButtonPure(zones, open, "explorer", "bottom", 0);
    expect(result.zones).toEqual({ top: ["test-view"], bottom: ["explorer"] });
  ...
  it("跨区 + 视图未打开 + 目标区已有打开视图：仅归属变化，目标区视图不被替换（R7）", () => {
    ...
    const zones: Zones = { top: ["test-view", "explorer"], bottom: ["search"] };
  ...
  it("同区移至同位：顺序不变，open 不变", () => {
    ...
    const open: OpenState = { top: "test-view", bottom: null };
    // 移到自己所在位置
    const result = moveButtonPure(zones, open, "test-view", "top", 0);
  ...
  it("index clamp：超限 clamp 到数组长度", () => {
    const zones: Zones = { top: ["test-view", "explorer"], bottom: [] };
    const open: OpenState = { top: null, bottom: null };
    const result = moveButtonPure(zones, open, "test-view", "top", 999);
    expect(result.zones).toEqual({
      top: ["explorer", "test-view"],
      bottom: [],
    });
...
describe("deriveLayout", () => {
  ...
  it("仅上区打开 → single-top（R4）", () => {
    expect(deriveLayout({ top: "test-view", bottom: null })).toBe("single-top");
  });
  ...
  it("双开 → split（R5）", () => {
    expect(deriveLayout({ top: "test-view", bottom: "explorer" })).toBe("split");
  });
});
...
describe("reconcileZones", () => {
  const registeredIds = ["test-view", "explorer", "search"];

  it("过滤 saved 中已注销的 id，保留 saved 顺序", () => {
    const saved: Zones = {
      top: ["test-view", "obsolete", "explorer"],
      bottom: [],
    };
    const open: OpenState = { top: "test-view", bottom: null };
    ...
    // open 仍指向 test-view（test-view 在 reconcile 后 zones 中）
    expect(result.open).toEqual({ top: "test-view", bottom: null });
  });

  it("缺失的注册 id 追加上区末尾（R9）", () => {
    const saved: Zones = { top: ["test-view"], bottom: [] };
    const open: OpenState = { top: "test-view", bottom: null };
    const result = reconcileZones(saved, open, registeredIds);
    expect(result.zones).toEqual({
      top: ["test-view", "explorer", "search"],
  ...
  it("入参数组不被 mutate（纯函数语义）", () => {
    ...
    const saved: Zones = { top: ["test-view"], bottom: ["obsolete"] };
    const open: OpenState = { top: "test-view", bottom: null };
    // 调用前快照（深拷贝内容）
    const before: Zones = {
      top: [...saved.top],
...
describe("场景序列（S1–S6）", () => {
  // 初始：P、E 均在上区，均关闭
  const initialZones: Zones = { top: ["test-view", "explorer"], bottom: [] };
  ...
  it("S1 基础开关——P 开→E 替换→E 关→侧栏隐藏", () => {
    ...
    // 1. 点击 P → 全高展示
    open = toggleViewPure(initialZones, open, "test-view");
    expect(open).toEqual({ top: "test-view", bottom: null });
    ...
    expect(deriveLayout(open)).toBe("single-top");
    // 验证无历史记忆：test-view 不出现在返回中
    expect(open.top).not.toBe("test-view");
  ...
  it("S2 双视图分区——上 P 下 E：全高→分区→恢复全高", () => {
    ...
    // 1. 点击 P → 全高展示（R4）
    open = toggleViewPure(zones, open, "test-view");
    expect(open).toEqual({ top: "test-view", bottom: null });
    expect(deriveLayout(open)).toBe("single-top");

    // 2. 点击 E → 上半 P、下半 E（R5）
    open = toggleViewPure(zones, open, "explorer");
    expect(open).toEqual({ top: "test-view", bottom: "explorer" });
  ...
  it("S3 同区替换——nav 打开中点击 E，E 替换 nav", () => {
    const zones = zonesBothTop();
    let open: OpenState = { top: "nav", bottom: null };

    // 1. 点击 E → E 打开、nav 关闭，全高展示文件浏览器（R1）
    open = toggleViewPure(zones, open, "explorer");
    expect(open).toEqual({ top: "explorer", bottom: null });
    expect(deriveLayout(open)).toBe("single-top");
  ...
  it("S4 拖拽换区 + 视图跟随——P 从上到下，E 被关闭", () => {
    let zones: Zones = { top: ["test-view"], bottom: ["explorer"] };
    let open: OpenState = { top: "test-view", bottom: "explorer" };
    expect(deriveLayout(open)).toBe("split");

    // 1. 将 P 从上区拖到下区 → P 立即移到下区展示
    //    下区原有的 E 被关闭（R6）；仅 P 一个视图打开 → 全高（R4）
    const result = moveButtonPure(zones, open, "test-view", "bottom", 0);
    zones = result.zones;
    open = result.open;

    expect(zones).toEqual({ top: [], bottom: ["test-view", "explorer"] });
    expect(open).toEqual({ top: null, bottom: "test-view" });
    expect(deriveLayout(open)).toBe("single-bottom");
  });

  it("S5 拖拽未打开按钮——E 从上到下仅归属变化", () => {
    let zones: Zones = { top: ["test-view", "explorer"], bottom: [] };
    let open: OpenState = { top: "test-view", bottom: null };
    expect(deriveLayout(open)).toBe("single-top");
    ...
    // 仅归属变化，展示不变（R7）
    expect(zones).toEqual({ top: ["test-view"], bottom: ["explorer"] });
    expect(open).toEqual({ top: "test-view", bottom: null });
    expect(deriveLayout(open)).toBe("single-top");
