...
HTML 内容通过 `<iframe sandbox="allow-scripts" srcDoc={...}>` 渲染（**不含 `allow-same-origin`**）。注入脚本包含两部分：

**键盘转发**：`keydown` capture → `window.parent.postMessage({type:"slterm_key", fingerprint, ...})`。父窗口 `handleMessage` → `exportContextBindings("global")` 比对 → 命中则 `window.dispatchEvent(合成KeyboardEvent)` → `ShortcutRegistry` 分发。

**片段链接拦截**：`click` capture → 检测 `<a href="#...">` → `preventDefault` + `scrollIntoView` + `try { location.hash = ... }`。WebView2 sandboxed iframe 不支持 `#fragment` 导航（srcdoc→跳父 URL、blob→"Not allowed to load local resource"），故必须 JS 拦截。

**限制**：`location.hash` 在 WebView2 sandbox 中可能被拦截（静默 try/catch），`:target` CSS 伪类不触发。需用户 HTML 用 JS class 切换替代 `:target`。
...
旧路径 `attachGlobalShortcutForwarder(doc)` 保留供将来使用。
