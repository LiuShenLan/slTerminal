...
`keyboard.ts` 全局 capture 阶段拦截 `Ctrl+Shift+C/V`（复制/粘贴），但 `Ctrl+C` 永不拦截——xterm.js 自然发送 `\x03` 到 PTY，claude 用它取消操作。
...
| `terminal/useXterm.ts` | xterm.js 完整生命周期 hook：创建实例、WebGL/detect、PTY spawn、ResizeObserver、输出 requestAnimationFrame 合帧、React StrictMode 双挂载守卫、E2E 钩子 |
| `terminal/keyboard.ts` | 全局键盘处理器：焦点终端跟踪、Ctrl+Shift+C/V 复制粘贴、Ctrl+C 保留、IME 合成透传 |
...
| `editor/useCodeMirror.ts` | CodeMirror 6 生命周期 hook：创建 EditorView、语言扩展、Ctrl+S 保存/G3 另存为、外部文件改动监听、脏状态跟踪 |
...
- **IPC 边界**：`terminal/useXterm.ts` 通过 `ipc/pty` 调 spawn/write/resize/kill；`editor/useCodeMirror.ts` 通过 `ipc/fs` 读写文件、`ipc/git` 获取 diff、`ipc/dialog` 弹另存为；`terminal/keyboard.ts` 通过 `ipc/clipboard` 读写剪贴板
