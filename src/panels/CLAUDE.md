...
- **hooksConfig** — hooks 配置面板（F6）：双模式编辑（JSON 优先 / GUI 表单）settings.json 的 hooks 子树（user/project/local 三层），单条启停（ADR-0002），F2 注入入口并入
...
### hooksConfig：双模式编辑（JSON/GUI）+ 单条启停（ADR-0002）
...
- **保存安全**（P3-FE-17）：JSON.parse + `validateHooksJson`（json-schema-library Draft07，非 ajv）双校验，任一失败弹窗拒绝写盘 → `filterDisabled` 剔除禁用条目 → 写盘；成功后提示「hooks 改动需重启 claude 会话生效」。
- **单条启停**（ADR-0002，P3-FE-19/20）：禁用状态存 slTerminal 侧 settings（`stores/hooksConfig` 的 `disabledHooks` 段，四元组 layer+event+matcher+command），**不出现在配置文件中**；保存时 `filterDisabled` 剔除（key.command 非空 → 剔该 command 型 handler；空串 → 剔整 matcher 组；组空 → 移除事件键）；失配记录（外部修改/手动改 JSON 致四元组找不到匹配）UI 标记「失效的禁用记录」而非静默丢弃，点击启用即删除记录。
- **注入段保护**（C13-8）：`command` 含 `slterm-hook-reporter` 子串的条目（`isSltermManaged`，识别规则照 C9）GUI 标记「slTerminal 托管」+ 禁删/禁禁用/表单只读；**JSON 模式不限制**（用户对自己文件有最终权利）。
...
- **`useCommandDetection`**：OSC 133 C 触发时调用 `onTabStateChange({ active: true, title, icon: "🟡" })`；OSC 133 D 重置
- **`useXterm`**：新增 `onHookEvent` 订阅 → 按 `panelId` 过滤 → `eventToStatus(event, notificationType?)` 纯函数（`src/lib/claudeStatus.ts`）→ `onTabStateChange({ active: true, icon: emoji })`；`SessionEnd` 调 `{ active: false }`
- **`TerminalPanel.handleTabStateChange`**：`active=true` 时只有 `title` 存在才 `setTitle`，只有 `icon !== undefined` 才 `updateParameters`；`active=false` 恢复原标题并清 icon
...
| `terminal/useCommandDetection.ts` | OSC 133 命令边界检测 + TabTitleRegistry 匹配 + 🟡 attention 指示（OSC 133 C 触发 `onTabStateChange({ active: true, icon: "🟡" })`） |
| `terminal/webgl.ts` | `detectWebgl()` + `setupWebglWithRetry()` 纯函数 |
| `terminal/useXterm.ts` | 编排层（~420 行），组合上述 6 个 hook + `src/lib/useFontSizeWheel`（Ctrl+Wheel 字体缩放）+ `onHookEvent` 订阅（按 panelId 过滤 → `eventToStatus` → F3 四态 emoji），对外接口兼容 TerminalPanel |
...
| `hooksConfig/HooksConfigPanel.tsx` | 面板根组件：层级切换器（user/project/local + 优先级标注，rootPath 空时 project/local 禁用）+ 模式切换（GUI/JSON，非法 JSON 禁 GUI）+ F2 注入状态条与注入/卸载按钮 + 失效禁用记录条 + 保存按钮（dirty 且合法才可点）+ 重启提示条；三态（loading/content/损坏 error） |
| `hooksConfig/useHooksConfig.ts` | 数据 hook：rootPath 推导（照 useCommitStatus）、`readHooksConfig` 加载（null 视为 {}，Err 置损坏态）、双模式同步（configJson/guiModel/dirty）、保存（双校验 + filterDisabled + writeHooksConfig）、切层/focusin 轻量重读（dirty `dialog.ask` 守卫 + generation 取消）、启停切换（disabledKeys/staleDisabledKeys/toggleDisable）；挂载时加载禁用状态 store |
| `hooksConfig/configModel.ts` | 配置模型双向转换纯函数：`jsonToGui`/`guiToJson`（round-trip 不丢数据，未知字段归 extraFields）、`isSltermManaged`（注入段识别，C9）、`isHandlerDisabled`/`isKeyPresentInConfig`/`filterDisabled`（四元组剔除规则，C13-8/ADR-0002） |
...
| `hooksConfig/EventTree.tsx` | 事件树（P3-FE-13）：三级树（分组折叠 → 事件 → matcher 组 → handler 摘要）+ hook 总数 + 仅渲染配置中已存在事件（未知事件归「未知事件」组）+ 「slTerminal 托管」标记 + 启停 checkbox（注入段无 checkbox——C13-8 禁禁用） |
| `hooksConfig/HandlerForm.tsx` | handler 编辑表单（P3-FE-14）：5 种 type 专用字段（字段矩阵来自 eventsCatalog 单一真值源）+ 通用字段（if/timeout/statusMessage，once 不展示）+ type 切换保留通用字段清除不适用字段 + 注入段只读/禁删/禁禁用 |
...
- **纯函数层**：`hooks-config-catalog`（eventsCatalog 事件元数据全表）、`hooks-config-matcher`（matchHook 全分支 + 受限窄字符集）、`hooks-config-model`（jsonToGui/guiToJson 双向转换 + isSltermManaged + filterDisabled）、`hooks-config-store`（disabledHooks sanitize/启停/debounce 持久化）
- **组件层**：`hooks-config-panel`（三态/层级切换器禁用逻辑/保存按钮/focusin 重读）、`hooks-config-jsonmode`（CM6 + schema 扩展注册/校验上报/事件导航/MatcherTester）、`hooks-config-gui`（Master-Detail 渲染/增删事件与 handler/选中守卫）、`hooks-config-handlerform`（5 种 type 字段矩阵断言/type 切换/注入段禁改）、`hooks-config-disable`（禁用→保存过滤/失效记录/事件树 checkbox）、`hooks-config-entry`（入口命令同页单例）、`hooks-config-sync`（双模式同步）
