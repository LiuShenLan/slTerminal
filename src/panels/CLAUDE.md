...
- **为何靠全局 CSP**：`<iframe srcDoc>` 加载的 `about:srcdoc` 是 local scheme，按 WHATWG/CSP3 规范**继承父窗口 CSP**，无法通过子文档自注入宽松 meta、去 `allow-same-origin` 或改 blob:/data: 绕过——子策略只能收紧不能放宽。故必须放宽主窗口 CSP。
