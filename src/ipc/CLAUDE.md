...
| `claudeHistory.ts` | `claude_history/` | `claude_history_scan`, `claude_history_delete`, `claude_history_rename`（claude 历史会话查询与恢复三命令）：`scanHistory()` 扫描全部历史会话（返回 `HistorySession[]`，单文件失败降级条目、扫描根不存在返回空数组均非 Err）；`deleteHistorySession(sessionId)` / `renameHistorySession(sessionId, newTitle)` 按 sessionId 定位文件（**前端不传路径，后端 SEC-01 校验**），invoke 参数 camelCase（JS `sessionId`/`newTitle` ↔ Rust `session_id`/`new_title` 由 Tauri 自动转换） |
