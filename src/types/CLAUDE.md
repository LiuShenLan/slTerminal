...
| `agent.ts` | `hooks/`（MC-212 决策 3 更名，原 `hooks.ts`） | `AgentEventPayload`（八字段 + 可选 `cliId`）、`ContextUsage`（inputTokens/outputTokens/cacheReadInputTokens/cacheCreationInputTokens）、`AgentInjectionStatus`、`AgentHookInjectionStatus` |
