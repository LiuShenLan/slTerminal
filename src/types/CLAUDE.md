...
| `hooks.ts` | `hooks/usage.rs` + signal.rs | `ContextUsage`（inputTokens/outputTokens/cacheReadInputTokens/cacheCreationInputTokens） |
...
| `claudeHistory.ts` | `claude_history/mod.rs` | `HistorySession`（七字段）、`TitleSource`（五变体） |
