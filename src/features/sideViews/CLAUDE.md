...
| `../agentStatus/AgentStatusView.tsx` | `agent-status` 视图组件（F7 三下拉框结构）：活跃会话（`useAgentStatus` + `AgentStatusRow`，逻辑零改动）+ 当前项目历史会话 + 全部项目历史会话（挂载 `ClaudeHistorySections` 受控区）；默认活跃展开、两历史区收起；E2E 兼容红线（`agent-status-view`/`agent-status-row`/"AGENT STATUS"/空态文案）逐字保留 |
