...
- **图标列 40px flex 簇**：emoji 与 CLI logo（16×16，`cliProfileRegistry.get(row.cliId)?.iconSrc`——**按行 cliId 查 profile**（MC-411），OSC 133-only 行同样有 cliId（logo 可展示），未注册 cliId → undefined → 无 logo 不报错）列内居中成组（列内 gap 4px）；`{icon}` 为空仍渲染空列占位（行1 标题起点恒定不漂移）；**F9 行为修订：logo 跟随会话名显示——行存在即显示，不依赖 emoji（status=null 行同样有 logo）**
- **行2 缩进 48px**（= 图标列 40 + gap 8），用量条与行1 标题起点对齐

**用量口径（官方 used_percentage，原 transcript token 累加 ÷ 硬编码 contextLimit 已退役）** = ContextUsage 信号（statusline 桥接通道，payload `usedPercentage`）→ 经 `cliProfileRegistry.get(row.cliId)?.capabilities?.hooks?.computeUsagePercent(row.usage)` 委托（claude = round + clamp 0–100；profile 缺失/无 hooks 能力/无数据 → `--`，MC-412 语义保留）。**用量条四档分级**：≥90 critical 红 / ≥70 high 橙 / ≥50 medium 黄 / else low 绿（`AGENT_STATUS_USAGE_COLORS` 四键）。时间口径与历史区统一（`formatRelativeTime`，见 @../agentHistory/CLAUDE.md）。
...
| `useAgentStatus.ts` | 数据 hook：`useAgentStatus()` → `AgentStatusResult`（`state` + `rows: AgentSessionRow[]` + `now` 时间基准）+ TerminalRegistry 订阅（register/remove/sessionChange）+ ContextUsage 信号事件处理（行 usage 更新） |
...
- `agent-status-view.test.tsx` — AgentStatusRow 双行布局 + AgentStatusView 三区结构 + 官方 used_percentage 口径（round/clamp 边界）+ 四档分级边界
