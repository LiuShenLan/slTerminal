...
- **行 cliId（MC-410）**：hook 事件通道建行按 MC-205 三级解析（`payload.cliId` → `TerminalRegistry.get(panelId)?.agentSession?.cliId` 反查 → `CLAUDE_CLI_ID` 缺省）写入；OSC 133/sessionChange 通道建行取 `agentSession.cliId`（缺省兜底）；无 hooks 能力或未知 cliId → `console.warn` + 跳过（不建行/不置图标/不通知，MC-206）
