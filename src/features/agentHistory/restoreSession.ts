...
async function doRestore(
  ...
  api.addPanel({
    ...
    // 读不到（title null）兜底 = sessionId 前 8 位（与 NavHistoryRow /
    // useXterm 同步兜底同口径）——运行中由 SessionStart 异步标题通道保持同步
  ...
  // 恢复命令前缀或吞首字节。cmd 无 shell integration（133;A 永不到达）→
  // 固定延迟兜底；超时兜底仍注入（不劣于无闸门现状）
