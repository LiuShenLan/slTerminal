...
- 三钮 = 最小化（`IconMin`）/ 最大化/还原（`IconMax`）/ 关闭（`IconCloseWin`），38×26、图标 12px、`aria-label` + `title`；hover 底 `ui.secondaryBg`（**关闭钮例外**：hover 底 = `TITLEBAR_CLOSE_HOVER_BG`（`ui.titlebarCloseHover` 危险色 `#c04747`，UI-301 定值，FE-07 token 化））
...
- **窄订阅（FE-21）**：标题推导经 `useShallow` 包装的 projects 窄 selector（返回 `{projectName, pageName}` 两原始值字段）——无关项目变更（layout/version/expandedNodes 等）结果浅相等，不触发 TitleBar 重渲染；`useShallow` 包装器每次渲染重建（zustand v5 无 useCallback 缓存），`activePageId` 闭包恒最新，布局切换响应不变
- **视觉**：34px 高、`TITLEBAR_BG`（明度阶梯 l2）底 + 底部 1px `SEPARATOR_BG` 发丝线、12px 文字、userSelect none；左段 app 标识 = accent-dim 底（`ACTIVE_SELECTION_BG`）+ `ACCENT_FG` 终端提示符 SVG（lucide Terminal path，与 final-mockup 一致）+「slTerminal」500 字重；全部颜色经 `theme/colors.ts` token（硬约束 #6，含关闭钮 hover 危险色——FE-07 已 token 化）
...
L2 测试：`src/__tests__/title-bar.test.tsx`（11 用例，TB-06-1~9 + FE-21-1/2，用例数见 `.claude/test-inventory.md`）：
...
- FE-21-1 无关项目变更不触发重渲染（标题不变）——Profiler onRender 计数 = TitleBar 子树提交次数，新增无关项目（proj-2）后断言无新提交且标题不变（窄订阅守卫，FE-21）
- FE-21-2 切换布局活跃页（layout store）后标题响应更新——`useLayout.setState` 切 activePageId → 中段标题跟随切换（窄订阅不破坏切换响应）
