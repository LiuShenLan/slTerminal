// =====================================================================
// Fix Loop — 妥协修复 review 问题修复计划（docs/compromises-fix-review-fix/）
// =====================================================================
...
//   fixContext: string  — verify agent 给出的 details 证据原文
//   verifyFile: string  — 断言清单路径（与 Stage 脚本同一真值源）
//   constraints: string — Stage 特殊纪律（取值见 execution-plan.md「fix-loop args 规范」表）
// 命令集说明：静态 + L1 + L2（不跑 L4/e2e——修复循环不承重 L4 时长；
//   L4 相关项修复后由主 agent 重跑对应 Stage 门禁补验）
// =====================================================================
...
纪律：
- 先读 docs/compromises-fix-review-fix/checklist.md 中对应 ID 的六段式条目 + docs/compromises-fix-review-fix/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
- 只修复上述列出的项，不顺手改无关代码（surgical changes）；代码注释用中文
- 禁区：compute_conpty_flags 固定 0x7，任何 agent 不得修改 ConPTY flags（含其 4 条守卫测试）——PASSTHROUGH_MODE (0x8) 吞 claude TUI 鼠标滚轮输入
- L1 定向红线（TQ-COV-06）：cargo 定向测试一律 \`cargo test --test lib_tests <filter> -- --test-threads=1\`，禁裸 filter/--lib（触发 0xc0000139）
...
// === Phase 2: 全量测试（逐条串行执行——CP-007 基准负载敏感，禁并行）===
phase('全量测试')
const testResult = await agent(`
在项目根目录 D:/data/learn/code/slTerminal 执行全量验证。以下命令**逐条串行执行**（前一条 exit 后再起下一条；L1/L2 禁并行——CP-007 基准对 CPU 负载敏感，并行抢核会误红）：
1. npx tsc --noEmit
2. npx eslint src/
...
// agent() 未返回时兜底——保留原 failedItems（不丢修复现场）
