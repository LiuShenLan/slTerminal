// =====================================================================
// fix-loop.js — Stage 验证不通过时的修复循环（由主 agent 循环调用，最多 3 轮）
// 本任务：docs/compromises-fix-review2-fix/
// 用法: Workflow({ scriptPath, args: { stage, failedItems, fixContext, verifyFile, constraints } })
//
// args 说明:
...
//   verifyFile: string — 断言清单文件路径（必填，如 docs/compromises-fix-review2-fix/workflows/verify/stage-01.md）
//                        ——与 Stage 脚本同一真值源；其「全量测试」节即本循环测试门禁（命令单一真值源）
//   constraints: string — Stage 特殊纪律（S01-S05 传空串；S06 传「只追加注记，禁改任何生产逻辑/原文改写」；
//                        S07 传「只改文档/注释/测试文案，禁改生产逻辑」——值见对应 Stage 脚本头注释）
// 串行纪律：用户并发限制持续有效——全量测试命令逐条串行执行，禁并行（CP-007 基准负载敏感）。
// =====================================================================
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/compromises-fix-review2-fix/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/compromises-fix-review2-fix/checklist.md 中对应 ID 的六段式条目 + docs/compromises-fix-review2-fix/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
- 只修复上述列出的项，不顺手改无关代码；代码注释用中文
- 禁区：compute_conpty_flags 固定 0x7，任何 agent 不得修改 ConPTY flags（含其 4 条守卫测试）——PASSTHROUGH_MODE (0x8) 吞 claude TUI 鼠标滚轮输入
- L1 定向测试红线：cargo test --test lib_tests <filter> -- --test-threads=1，禁 --lib/裸 filter（0xc0000139）
...
先读 ${VERIFY_FILE} 的「全量测试」节获取本 Stage 测试门禁命令清单（命令单一真值源），以下纪律执行：
- 命令**逐条串行执行，禁止并行**（CP-007 基准负载敏感，并行抢核曾致误红）
- cargo test 带 filter 一律走 cargo test --test lib_tests <filter> -- --test-threads=1（L1 定向红线）
