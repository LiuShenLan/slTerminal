// =====================================================================
// Fix Workflow — 设置中心（F11）阶段验证不通过时的修复循环
// =====================================================================
// 用法: Workflow({ scriptPath: 'docs/settings-center/workflows/fix-loop.js',
...
//   verifyFile: string — 断言清单文件路径（必填，如 docs/settings-center/workflows/verify/stage-01.md）
//                        ——与 Stage 脚本同一真值源，保证修复循环与 Stage 验证同一标尺
//   constraints: string — Stage 特殊纪律（可选；值见对应 Stage 脚本头注释「fix-loop 调用约束」）
//
// 门禁说明: 本脚本跑常规六级（tsc/eslint/clippy/rustfmt/npm test/cargo test）。
//   Stage 06 的修复循环若涉及 e2e spec/helpers，主 agent 须在 fix-loop 返回后
//   单独补跑 npm run e2e 全量（本脚本不含——e2e 十分钟级，不适合每轮修复循环）。
// =====================================================================
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/settings-center/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/settings-center/checklist.md 中对应 ID 的修复要点 + docs/settings-center/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
