...
export const meta = {
  name: 'fix-loop',
  description: '阶段 2 修复循环：针对 Stage 验证失败项修复并重验',
  phases: [
    { title: '修复问题' },
    { title: '全量测试' },
    { title: '逐项验证' },
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/hooks-dev/phase2/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/hooks-dev/phase2/checklist.md 中对应 ID 的修复要点 + docs/hooks-dev/phase2/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位。
