...
- 校验栈：`codemirror-json-schema`（悬停 `jsonSchemaHover` / 波浪线 `jsonSchemaLinter`；**无自动补全**——2026-08-01 验收后决策删除）+ 其底层 `json-schema-library` 做保存前独立校验（**`new Draft07(schema).validate(data)`**——2026-08-01 执行期确认 `json-schema-library@9.3.5` 无 `compileSchema` 导出，真实 API 为 `Draft07` 类构造 + `.validate`，见 `src/features/hooksConfig/schema/index.ts`）——**不引 ajv**。新增依赖：`codemirror-json-schema`、`@codemirror/lint`（`@codemirror/lang-json` 已有；`@codemirror/autocomplete` 已移除——补全删除后无直接引用）。
