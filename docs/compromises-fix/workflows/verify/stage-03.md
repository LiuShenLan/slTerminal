...
> 本 Stage 起 L1 定向命令维持旧形态 = `cargo test --test lib_tests <filter> -- --test-threads=1`（**S02 未拆红线**：embed-manifest 通道实测失实转登记，2026-09-07 执行期翻案）；文中裸 `cargo test <filter>` 断言按此形态执行或以全量（无 filter）覆盖判定。
