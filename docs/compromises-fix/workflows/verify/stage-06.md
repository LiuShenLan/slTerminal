...
- **CP-007-a（基准用例常驻）**：`src-tauri/src/agent_history/claude/scan.rs` 含 `scan_bench_1000_sessions_median_under_200ms` 且非 `#[ignore]`（放宽依据：2026-09-08 实测原始键实现中位 180.33ms 越原决策门槛 50ms → 走指纹分支后常驻门槛按 flaky 处置纪律放宽为中位 <200ms，数字留痕 scan.rs 注释与 commit body）；实测中位/max 数字已记录进 commit body（Read commit message 确认）。
