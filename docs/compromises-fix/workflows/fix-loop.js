// =====================================================================
// Fix Loop — 妥协修复计划 Stage 验证不通过时的修复循环
// 由主 agent 循环调用（每 Stage 上限见 config.json workflow.fixMaxRetries）：
//   Workflow({ scriptPath: 'docs/compromises-fix/workflows/fix-loop.js',
//              args: { stage, failedItems, fixContext, verifyFile, constraints } })
// args 说明：
//   stage: number        — Stage 编号（1-12，用于 label 与报告）
//   failedItems: string[]— 未通过项 ID 列表（必填非空，来自 Stage verify）
//   fixContext: string   — verify agent 的 details 证据原文
//   verifyFile: string   — docs/compromises-fix/workflows/verify/stage-NN.md
//   constraints: string  — Stage 特殊纪律（可选；取值唯一真值源 =
//                          docs/compromises-fix/execution-plan.md「fix-loop 调用规范」表）
// =====================================================================
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（docs/compromises-fix/workflows/verify/stage-NN.md）')
}
...
纪律：
- 先读 docs/compromises-fix/checklist.md 中对应 ID 的修复步骤 + docs/compromises-fix/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
- 只修复上述列出的项，不顺手改无关代码；代码注释用中文
- 禁区：compute_conpty_flags 固定 0x7，任何 agent 不得修改 ConPTY flags（含其 4 条守卫测试）——PASSTHROUGH_MODE (0x8) 吞 claude TUI 鼠标滚轮输入（Stage 08 执行期对 CP-009 单项豁免，其余 Stage 无豁免）
