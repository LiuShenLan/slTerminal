// =====================================================================
// Fix Workflow（配色重构任务）— 阶段验证不通过时的修复循环
// =====================================================================
// 用法: 主 agent 调用：
//         Workflow({ scriptPath: 'docs/color-plan/workflows/fix-loop.js',
//                    args: { stage, failedItems, fixContext, verifyFile, constraints } })
//
// args 说明:
//   stage: number      — 所属 Stage 编号（用于 label 与报告，并决定门禁命令集）
//   failedItems: string[] — 未通过项 ID 列表（必填，非空；来自 Stage verify）
//   fixContext: string — verify agent 给出的 details 证据原文（失败原因线索）
//   verifyFile: string — 断言清单文件路径（必填，如 docs/color-plan/workflows/verify/stage-01.md）
//                        ——与 Stage 脚本同一真值源，保证修复循环与 Stage 验证同一标尺
//   constraints: string — Stage 特殊纪律（可选；仅 Stage 05/06 有值，其余 Stage 不传——
//                        值唯一真值源 = 对应 Stage 脚本头注释的 fix-loop 调用约定，此处不复制）
//
// 门禁命令唯一真值源 = docs/color-plan/execution-plan.md Stage 编排总表；
// 下方 TEST_COMMANDS_BY_STAGE 为其机械转写（修改须先改 execution-plan 再同步此处与各 Stage 脚本）。
// =====================================================================
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/color-plan/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/color-plan/checklist.md 中对应 ID 的修复要点 + docs/color-plan/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
