// =====================================================================
// Fix Workflow — claude-history-view 修复循环
// 由主 agent 调用：
//   Workflow({ scriptPath: "docs/claude-history-view/workflows/fix-loop.js",
//              args: { stage, failedItems, fixContext, verifyFile, constraints } })
// args 规范（execution-plan.md 单处定义）:
//   stage: number — 所属 Stage 编号
//   failedItems: string[] — 未通过项 ID 列表（必填，非空）
//   fixContext: string — verify agent details 证据原文
//   verifyFile: string — docs/claude-history-view/workflows/verify/stage-NN.md（与 Stage 同一真值源）
//   constraints: string — 恒传空串，除两例外：
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/claude-history-view/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/claude-history-view/checklist.md 中对应 ID 的修复要点 + docs/claude-history-view/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
