// =====================================================================
// Fix Workflow — Stage 验证不通过时的修复循环（conda-profile-fix）
// =====================================================================
// 调用方式（主 agent 循环调用，最多 3 轮）：
//   Workflow({ scriptPath: 'docs/conda-profile-fix/workflows/fix-loop.js',
//              args: { stage, failedItems, fixContext, verifyFile, constraints } })
// args 规范见 docs/conda-profile-fix/execution-plan.md「fix-loop args 规范」表
// （constraints 取值以该表为唯一真值源，本文件不复制）。
// =====================================================================
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/conda-profile-fix/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/conda-profile-fix/checklist.md 中对应 ID 的修复要点 + docs/conda-profile-fix/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
...
逐条报告：每命令一行 exit code + 通过/失败；失败时附前 50 行错误摘要，勿贴完整输出。cargo 系命令共享 target 目录锁会排队，属正常勿中止。cargo test 报告末尾附全部 test result 行的 passed 总数汇总。
