...
- **FE-02**：`src/ipc/index.ts` 含 `export * as backgroundTasks from "./backgroundTasks";`；`src/ipc/planBalance.ts` 中 grep `setPlanBalanceInterval` 零命中（`getPlanBalance`/`refreshPlanBalance`/`onPlanBalanceUpdated` 保留）。
