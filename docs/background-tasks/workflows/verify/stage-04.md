...
- **FE-07**：grep `settings-plan-balance` 在 src/ 零命中（e2e-tests/ 由 Stage 05 处理，本断言限 src/；`src/__tests__/CLAUDE.md` 与 `.claude/test-inventory.md` 的迁移表同步属 Stage 06 DOC-01/02 职责，本 Stage 不判 not_fixed——跨 Stage 边界，2026-08-31 登记）。
