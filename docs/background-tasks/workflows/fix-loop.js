...
//   verifyFile: string — 断言清单文件路径（必填，如 docs/background-tasks/workflows/verify/stage-01.md）
//                        ——与 Stage 脚本同一真值源，保证修复循环与 Stage 验证同一标尺
//   constraints: string — Stage 特殊纪律（可选；Stage 05 传「只改 e2e-tests/ 下文件，不改 src/ 与
//                        src-tauri/ 生产代码；若失败根因在生产代码，报告并停止」，其余 Stage 传空串）
// =====================================================================
...
if (typeof args.verifyFile !== 'string' || args.verifyFile.length === 0) {
  throw new Error('fix-loop 需要 args.verifyFile（断言清单文件路径，如 docs/background-tasks/workflows/verify/stage-01.md）')
}
...
纪律：
- 先读 docs/background-tasks/checklist.md 中对应 ID 的修复要点 + docs/background-tasks/stages.md 中 Stage ${STAGE} 的实现要点，再结合失败证据定位
