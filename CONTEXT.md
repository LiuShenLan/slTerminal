...
**操作页面**（OperationPage）：
项目内一个独立的工作页，拥有自己的面板布局（共享宿主内一个页组，页内可拖拽分屏为多组——ADR-0020）。页面间显隐切换且终端跨页面存活（详见 workspace 模块文档）。
_Avoid_: 操作页, tab, 标签页
...
**工作区**（Workspace）：
多页面 Dockview 布局的根 UI 组件。管理单一共享 Dockview 宿主（页组模型：每操作页面 = 宿主内页组，ADR-0020 起页内可分屏多组）、页面切换、布局持久化和侧边栏布局。
...
应用级周期性执行的任务单元。元数据单点 = 后端任务注册表（taskId/标题/频率边界/默认值）；执行体在后端（poller 骨架驱动，如套餐余量查询）或前端（前端调度器驱动，如 session 刷新）；配置统一持久化于应用 settings.json `backgroundTasks` 段，设置中心「后台定时任务」页统一管启用与频率。新增任务 = 注册一条元数据 + 一端写执行体，框架与设置页零改动。

**扫描执行体**（Scan Executor）：
历史会话扫描的唯一执行路径——遍历全部已注册 history provider 逐个扫描并聚合。手动刷新（刷新钮 / triggerNow）与定时刷新（tick）同为它的触发器，两者仅失败处理策略不同。

**触发来源**（Trigger Source）：
任务执行体的触发渠道二分——`manual`（刷新钮 / triggerNow）与 `tick`（定时器）。**仅影响失败处理策略**（规格 §7）：tick 失败静默（快照不变），manual 失败置 error 态（保留旧 data）；执行体本身对两者无差别。
