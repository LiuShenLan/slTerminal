...
Claude Code 历史会话查询——扫描 `~/.claude/projects/` 下全部会话 transcript（JSONL）并暴露三命令：`claude_history_scan`（元数据扫描）、`claude_history_delete`（删除会话）、`claude_history_rename`（重命名会话）。需求规格见 `docs/claude-history-view/README.md`（v1.1，决策 22–26）。
...
`claude_history_delete` / `claude_history_rename` 入参 `session_id` 严格校验 UUID 形态（复用 `is_uuid_filename`：36 长度 + 连字符位置 + ascii hex 全检，天然拒绝含 `..`、路径分隔符、空串的一切输入）→ `AppError::Validation`。文件定位 = 遍历扫描根一级子目录找 `<session_id>.jsonl`（`locate_session_jsonl`），**前端不传任何路径**。两命令写用户 home 目录文件、绕过 project_root 沙箱（照 `hooks/config.rs` user 层先例），入参即攻击面。
...
### 写操作（BE-07/BE-08，ops.rs）

- **delete**：删 `<id>.jsonl` + 同名 `<id>/` 目录存在则 `remove_dir_all`（含 subagents 附属数据）；jsonl 不存在 → `AppError::Validation`（「会话不存在」语义）
- **rename**：newTitle trim 后非空且 ≤200 字符 → `OpenOptions.append` 追加一行 `{"type":"custom-title","customTitle":<名>,"sessionId":<id>}`（**serde_json 序列化构造，禁手拼字符串防注入**）。不做原子重写（追加写与运行中会话写入无冲突，决策 20/22）。追加前 `ensure_trailing_newline` 补换行保证 JSONL 行完整（append 句柄 seek 无效，用独立只读句柄探测末字节）

### 命令与并发约束

三命令均在 `lib.rs` 的 `generate_handler!` 注册（`claude_history::scan::claude_history_scan` / `claude_history::ops::claude_history_delete` / `claude_history::ops::claude_history_rename`）；阻塞 I/O 全部 `spawn_blocking`（硬约束 #3）；DTO 双边对应 `HistorySession` / `TitleSource`（serde camelCase，硬约束 #4）。扫描顺序无契约（不排序——排序/分组/过滤是前端 `src/features/claudeHistory/historyModel.ts` 的职责）。
...
| `ops.rs` | 写操作：`validate_session_id()`（SEC-01）+ `claude_history_delete`（BE-07）+ `claude_history_rename`（BE-08，custom-title 追加写） |

## 测试模式

Rust 测试 4 个位置（均为 `#[cfg(test)] mod tests` 嵌入源文件底部），共 62 用例（grep `#[test]` 口径，与 `.claude/test-inventory.md` 一致）：
...
| `ops.rs` | 13 | validate_session_id（UUID 双形态接受/5 类非法拒绝）、delete（jsonl+同名目录范围/仅 jsonl/不存在 Err/非法 id 端到端）、rename（追加行三字段逐字断言/补换行/空与超 200 拒绝且不改文件/恰 200 边界/不存在 Err/非法 id）、越界防护（扫描根外哨兵文件不被触碰） |
...
- **纯逻辑函数直测**：命令包装（async + spawn_blocking）不直测，直接调纯 I/O 逻辑（`scan_sessions` / `delete_session` / `rename_session` / `parse_head` / `parse_tail_title` / `resolve_title`）；命令注册与端到端由 L4 E2E 验收
...
4. 修改 `validate_session_id` / delete / rename 后跑 `ops.rs` 全部 13 条测试；**勿削弱 SEC-01 校验**（定位只接受 sessionId，前端不传路径）
5. env 测试依赖 `--test-threads=1` 门禁——勿在测试中引入并行 env 操作
6. 新增 Tauri 命令后在 `lib.rs` 的 `generate_handler!` 注册（本模块三命令已注册）
