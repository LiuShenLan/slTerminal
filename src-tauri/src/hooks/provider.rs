...
//! - `CliHooksProvider` trait 九方法——七方法签名写死 + `ensure_hooks_scripts`/
//!   `confirm_inject` 两方法带默认实现（9-6 启动对账钩子、CP-043 确认注入，既有实现零改动）
...
/// CLI hooks 能力 trait（九方法——七方法签名写死 + ensure_hooks_scripts/confirm_inject
/// 两方法带默认实现）
///
